...
if page == "Dashboard":
    ...
    for idx, row in alertes.iterrows():
        if st.button(f"Planifier rattrapage : {row['Chapitre']}", key=f"p_{idx}"):
            ...
            st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([{'Dossier': choix_dos, 'Matiere': row['Matiere'], 'Chapitre': row['Chapitre'], 'J_Type': 'Rattrapage', 'Date': prochaine, 'Note': 0}])], ignore_index=True)
    ...
    with st.expander("➕ Ajouter Chapitre"):
        with st.form("Add"):
            ...
            if st.form_submit_button("Générer planning"):
                if not date_exam: st.error("⚠️ Date d'examen obligatoire !")
                else:
