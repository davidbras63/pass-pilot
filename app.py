...
def load_data_from_sheet():
    try:
        ...
        if response.status_code == 200:
            ...
            # Modification unique pour bloquer la date (lecture)
...
def save_all_to_sheet(df, config):
    df_to_send = df.copy()
    # Modification unique pour bloquer la date (écriture)
    ...
    if not rattrapages.empty:
        ...
        for _, row in rattrapages.iterrows():
            if st.button(f"Réintégrer {row['Chapitre']}", key=f"btn_{row['ID']}"):
                ...
                new_r = row.copy()
                new_r['ID'], new_r['Chapitre'], new_r['Date'] = str(uuid.uuid4()), f"{row['Chapitre']}R", dt.date.today()
