...
window.addEventListener('beforeunload', function (e) {{
    const data = {st.session_state.data.to_json(orient='records') if 'data' in st.session_state else '[]'};
    const config = {json.dumps(st.session_state.config) if 'config' in st.session_state else '{}'};
    navigator.sendBeacon('{WEB_APP_URL}', JSON.stringify({{"data": data, "config": config}}));
}});
</script>
""")
