...
def load_data_from_sheet():
    try:
        ...
        if response.status_code == 200:
            ...
            # Verrouillage date : forcé en string pour empêcher toute conversion automatique
...
def save_all_to_sheet(df, config):
    df_to_send = df.copy()
    # Verrouillage date : forcé en string
    ...
    if not rattrapages.empty:
        ...
        for _, row in rattrapages.iterrows():
            if st.button(f"Réintégrer {row['Chapitre']}", key=f"btn_{row['ID']}"):
                ...
                new_row = row.copy()
                new_row['ID'], new_row['Chapitre'], new_row['Date'] = str(uuid.uuid4()), f"{row['Chapitre']}R", str(dt.date.today())
