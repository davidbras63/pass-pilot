...
def load_data():
    if os.path.exists(DATA_FILE):
        try:
            ...
            # On s'assure que 'Note' est numérique pour éviter les erreurs
...
with st.sidebar.expander("🛠️ Réglages", expanded=True):
    ...
    for j in st.session_state.config['cadencier']:
        key = str(j)
        st.session_state.config['seuils'][key] = st.slider(f"Seuil J{j}", 10, 20, int(st.session_state.config['seuils'].get(key, 12)))
...
if page == "Dashboard":
    ...
    rattrapages = df[(df['Note'] > 0) & (df['Note'] < 12)] # Exemple seuil basique
    ...
    if not df_clean.empty: st.line_chart(df_clean.pivot(index='Date', columns='Matiere', values='Note'))