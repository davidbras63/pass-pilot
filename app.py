...
def load_data_from_sheet():
    # Sécurité : On tente de charger, sinon on stoppe tout pour ne pas vider la RAM
    try:
        ...
        pass
    # Si échec, on arrête tout pour protéger tes données
    st.error("Impossible de récupérer les données depuis Google Sheets. Vérifiez votre connexion et rafraîchissez.")
    st.stop()

# --- SÉCURITÉ DE FERMETURE (ENVOI FINAL) ---
if 'data' in st.session_state:
    html(f"""
    <script>
    window.addEventListener('beforeunload', function (e) {{
        const payload = {{
            "data": {st.session_state.data.to_json(orient='records')},
            "config": {json.dumps(st.session_state.config)}
        }};
...
# --- LE RESTE DE TON CODE ORIGINAL (INCHANGÉ) ---
