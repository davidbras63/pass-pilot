...
with st.sidebar.expander("🛠️ Réglages", expanded=True):
    ...
    for j in st.session_state.config['cadencier']:
        key = str(j)
        st.session_state.config['seuils'][key] = st.slider(f"Seuil note J{j}", 10, 20, int(st.session_state.config.get('seuils', {}).get(key, 12)))
    if st.button("💾 Enregistrer Réglages"): save_config(st.session_state.config); st.rerun()

# --- GESTION DOSSIERS ET MATIÈRES ---
if "folder_key" not in st.session_state: st.session_state.folder_key = 0
if "mat_key" not in st.session_state: st.session_state.mat_key = 0
...
if st.sidebar.button("➕ Créer Dossier") and new_folder:
    ...
    save_config(st.session_state.config); st.session_state.folder_key += 1; st.rerun()

choix_dos = st.sidebar.selectbox("Dossier", list(st.session_state.config['dossiers'].keys()))

new_mat = st.sidebar.text_input("Ajouter Matière", key=f"m_{st.session_state.mat_key}")
if st.sidebar.button("Ajouter Matière") and new_mat: 
    st.session_state.config['dossiers'][choix_dos].append(new_mat)
    save_config(st.session_state.config); st.session_state.mat_key += 1; st.rerun()
