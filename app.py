...
with st.sidebar.expander("🛠️ Réglages", expanded=True):
    ...
    cad_input = st.text_input("Cadencier (ex: 1,3,7,14,30,60,90,120)", ",".join(map(str, st.session_state.config['cadencier'])))
    st.session_state.config['cadencier'] = [int(x.strip()) for x in cad_input.split(",")]
    
    st.write("---")
    st.subheader("Seuils de notes par J")
    for j in st.session_state.config['cadencier']:
        st.session_state.config['seuils'][str(j)] = st.slider(f"Seuil note J{j}", 10, 20, st.session_state.config['seuils'].get(str(j), 12))
