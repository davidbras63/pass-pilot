...
if st.sidebar.button("Supprimer dernière matière"):
    if st.session_state.dossiers[choix_dos]:
        st.session_state.dossiers[choix_dos].pop()
        st.rerun()
...
if page == "Dashboard":
    ...
    id_s = st.number_input("Entrez l'ID pour saisir la note", 0, len(st.session_state.data)-1 if not st.session_state.data.empty else 0)
    note = st.number_input("Note (/20)", 0, 20)
