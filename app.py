...
def load_config():
    ...
    return {'cours_max': 5, 'cadencier': [1, 3, 7, 14, 30, 60, 90, 120], 'seuils': {'1':12, '3':12, '7':14}, 'dossiers': {"PASS": []}}
...
new_f = st.sidebar.text_input("Nouveau Dossier")
if st.sidebar.button("➕ Créer Dossier") and new_f:
    st.session_state.config['dossiers'][new_f] = []; st.rerun()

choix_dos = st.sidebar.selectbox("Dossier", list(st.session_state.config['dossiers'].keys()))
new_m = st.sidebar.text_input("Ajouter Matière")
if st.sidebar.button("Ajouter Matière") and new_m: 
    st.session_state.config['dossiers'][choix_dos].append(new_m); st.rerun()
