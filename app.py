...
def save_data(df):
    df.drop_duplicates().to_csv(DATA_FILE, index=False)
...
# --- PLANNING & SAISIE ---
    ...
    if st.button("Enregistrer les notes"): st.session_state.data.update(edited); save_data(st.session_state.data); st.rerun()
