    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f: 
                cfg = json.load(f)
                # Sécurité : si la config est incomplète, on ajoute les dossiers par défaut
                if 'dossiers' not in cfg: cfg['dossiers'] = {"PASS": ["UE1", "UE2"]}
                return cfg
...
# --- SÉLECTION DOSSIER SÉCURISÉE ---
dossiers_dispo = list(st.session_state.config['dossiers'].keys())
choix_dos = st.sidebar.selectbox("Dossier", dossiers_dispo)
